...
export async function sendNewPassword(email, password) {
  return await transporter.sendMail({
    from: process.env.EMAIL_USER,
    to: email,
    subject: 'Nueva contraseña de Estrella Service',
...
export const createChangePassword = (password) => {
  ...
  .firma {
    ...
    <body>
    <h1>Tu nueva contraseña de Estrella Service</h1>
    <p>Esta es tu nueva contraseña para acceder a Estrella Service:${password} , una vez hayas accedido de nuevo, puedes cambiarla en cualquier momento desde la página de tu perfil.</p>
    <p>Vuelve al inicio de sesión <a href="https://www.estrellaservice.com" target="_blank" rel="noopener noreferrer">haciendo clic aquí</a>.</p>
    <br/>
      <p class="firma">The Estrella Service Team</p>
  </body>
  </html>`;
